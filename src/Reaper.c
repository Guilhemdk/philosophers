...
static int	philo_died(t_philo *philo)
{
	...
	if (mutex_get_int(&philo->philo_lock, &philo->full))
		return (0);
	elapsed = get_time(MILLISECONDS) - mutex_get_long(&philo->philo_lock,
			&philo->last_meal);
...
void	*fk_em_up(void *data)
{
	...
	while (!all_threads_ready(&program->program_lock, &program->threads_running,
			program->nbr_of_philos))
		;
	while (!end_of_simulation(program))
	{
		i = -1;
		while (++i < program->nbr_of_philos && !end_of_simulation(program))
		{
